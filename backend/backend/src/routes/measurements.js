...
// GET /measurements — latest + history
router.get('/', async (req, res) => {
  ...
  const history = await q(
    `SELECT * FROM body_measurements WHERE user_id=$1 ORDER BY created_at DESC LIMIT 20`,
    [uid(req)]
  );
  res.json({ latest: latest.rows[0] || null, history: history.rows });
});

// POST /measurements { waist, hips, chest, arms, weight }
router.post('/', async (req, res) => {
  const { waist, hips, chest, arms, weight } = req.body || {};
  const r = await q(
    `INSERT INTO body_measurements (user_id, waist, hips, chest, arms, weight)
     VALUES ($1,$2,$3,$4,$5,$6) RETURNING id, created_at`,
    [uid(req), waist || null, hips || null, chest || null, arms || null, weight || null]
  );
  // Award XP for tracking measurements
